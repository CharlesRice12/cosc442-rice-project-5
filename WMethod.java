...
public class WMethod{
   ...
   */
   public static void main(String [] args){
     ...
     try {
     FileWriter test = new FileWriter("tests.txt");
     PrintWriter p = new PrintWriter(test);
     while(tests.iterator().hasNext())
    	 p.println(tests.iterator().next());
     test.close();
     }catch (IOException e) {
    	 e.printStackTrace();
     }
     ...
     for(int i=0; i < 28 ; i++) {
    	 ...
    	 Utilities.runFSM(FSM,FSM[k].getID(),tests.elementAt(i)," ");
    	 k++;
     }
